...
int Lista::operator[](int i)
{
    ...
    Nod* p = start;
    if (i >= size || contor > i)        //daca pozitia dorita este mai mare decat lungimea listei sau daca este un nr.negativ
    {
        throw std::out_of_range("index gresit");
    }
    if (contor == i)                    //daca ni se cere chiar elementul de pe pozitia 0
    {
...
int Lista::cautare(int x)           //functia returneaza prima pozitie pe care a fost gasit elementul x
{
    ...
    if (p != NULL)
    {
        ...
        throw std::logic_error("Elementul nu exista");                //in cazul in care nu gasim elementul;
    }
    else {
        throw std::out_of_range("index gresit");//Lista este vida,dorim sa generam o eroare,deoarece nu putem cauta un element intr-o lista goala
    }
}
void Lista::remove(int x)
{
    ...
    if (i >= 0 && i < size)              //realizam stergerea atata timp cat elementul exista in lista
    {
        if (size == 1)         //daca este singurul element atunci lista devine vida
        {
            Nod* p = start;
            delete p;
            start = NULL;
        ...
        throw std::logic_error("Elementul nu exista");
    }
}
int Lista::elemMaxim()
{
    ...
    if (size == 0)
    {
        throw std::length_error("Lista vida,nu exista un element maxim");
    }
    if (size == 1)
...
int Lista::elemMinim()
{
    ...
    if (size == 0)
    {
        throw std::length_error("Lista vida,nu exista un element minim");
    }                                        //daca lista este vida simulam o eroare
    if (size == 1)
...
int Lista::suma()
{
    ...
    if (p != NULL)                     //daca lista nu este vida calculam suma
    {
        ...
        throw std::length_error("Lista vida");
    }                                                //in caz contrar
}
...
Lista& Lista::operator*(const int x)
{
    if (size == 0)
    {
        throw std::domain_error("Lista vida");
...
void Lista::reverse()
{
    if (size == 0)
    {
        throw std::length_error("Lista vida"); //daca lista este vida aruncam o eroare
    }
